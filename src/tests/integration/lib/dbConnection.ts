...
export class DBConnection extends EventEmitter {
    ...
    public constructor(
        ...
        super();
        this.ports = options.ports ?? DEFAULT_TEST_PORTS;
    ...
    public readonly ports: Readonly<TestPorts>;
    ...
    public pushMessage(instanceId: string, msg: any, callback: (err: Error | null, id: any) => void): void {
        if (!this._statesClient) {
            throw new Error('States DB is not running');
        }
