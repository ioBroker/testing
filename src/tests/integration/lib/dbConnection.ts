...
export class DBConnection {
	...
	public async readObjectsDB() {
		...
		if (await pathExists(this.objectsPath)) {
			// debug(`  exists:      true`);
			return readJSON(this.objectsPath, { encoding: "utf8" }) as Promise<Record<string, ioBroker.Object>>;
		}
	}

	public async writeObjectsDB(objects: ObjectsDB | undefined) {
		if (!objects) return;
		return writeJSON(this.objectsPath, objects);
	}

	public async writeStatesDB(states: StatesDB | undefined) {
		if (!states) return;
		return writeJSON(this.statesPath, states);
	}

	public async readStatesDB() {
	 ...
	 * @param testDir The directory the integration tests are executed in
	 */
	public async writeDB(objects: any, states: any) {
