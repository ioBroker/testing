...
-->
## **WORK IN PROGRESS**
* (@krobipd) The io-package.json check accepts the release-script's `NEXT` placeholder in `common.news` (it is replaced by the version number on release), so `npm test` no longer fails between two releases
